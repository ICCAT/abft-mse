...
setMethod("initialize", "OM", function(.Object,OMd="C:/M3",nsim=32,proyears=30,seed=1){
  ...
  if(class(OMd)=='OMd'){
    ...
    dimslots<-slotNames(OMd)[1:15]
    for(i in 1:15)slot(.Object,dimslots[i])<-slot(OMd,dimslots[i])
    ...
    .Object@q<-(totF*relF)/.Object@nsubyears
    ...
    .Object@Idist<-.Object@MIdist<-array(NA,c(1,1))
    
    # calcmovements 
    movs<-samps[,grep("movest",vcv$names)]
    # movs[1,]<-vcv$est[grep("movest",vcv$names)]
    mov<-array(NA, c(nsim,npop,nages,nsubyears,nareas,nareas))
    movcalc<-array(-10,c(nsim,npop,nsubyears,nareas,nareas))
    for(ss in 1:nsim){
      
      if(out$movtype==1){   # gravity model
        # areas that gravities are fixed to zero for
        # p s tr
        ind<-cbind(rep(ss,nareas*out$nmov1),
                   rep(out$mov1[,1],each=nareas),
                   rep(out$mov1[,2],each=nareas),
                   rep(1:nareas,out$nmov1),
                   rep(out$mov1[,3],each=nareas))
        movcalc[ind]<-0;
        
        refind<-cbind(rep(ss,out$nmovind*nareas),rep((1:out$nmovind),each=nareas)+npop*nsubyears)
        ...
        ind<-TEG(c(nareas,nsubyears,npop))[,c(3:1,1)]
        refind<-cbind(rep(ss,nrow(ind)),(ind[,1]-1)*nsubyears+ind[,2])
        ind<-cbind(rep(ss,nrow(ind)),ind)
        movcalc[ind]<-movcalc[ind]+exp(movs[refind])
    ...
    for(ss in 1:out$nsel){
      ...
      if(out$seltype[ss]==2){
        spar2<-ml[nlen]*(0.1+0.7*exp(spars[,2])/(1+exp(spars[,2])));       #// Inflection point (2) as a fraction of largest length I(0.1|0.8)
        spar1<-spar2*(0.01+0.49*(exp(spars[,1])/(1+exp(spars[,1]))));  # // Logistic slope (1) as fraction of inflection point (2) I(0.01|0.5)
        ...
        spar2<-0.1+(0.6*exp(spars[,2])/(1+exp(spars[,2])));               #// Precision as the ratio of the inflection point I(0.1|0.7)
        spar3<-ml[nlen]*(0.1+(0.8*exp(spars[,3])/(1+exp(spars[,3]))));   #// Inflection point as a fraction of largest length I(0.15|0.9)
        ind<-as.matrix(expand.grid(1:nsim,ss,1:nlen))  
        inds<-ind[,1]
        indl<-ind[,3]
...
setMethod("initialize", "SimSam", function(.Object,OM,Obs,movtype=2,OMDir="G:/M3",verbose=0,complexF=0,complexRD=0){
 #.Object})
 #.Object<-new('SimSam')
  ...
  # copy over dimensions ------
  dimslots<-slotNames(OM)[1:15]
  for(i in 1:15)slot(.Object,dimslots[i])<-slot(OM,dimslots[i])
  ...
  RFL<-array(NA,c(nsim,nfleets,nlen,nyears,nareas))
  indL<-as.matrix(expand.grid(1:nsim,1:nfleets,1:nlen,1:nyears,1:nareas))
  RFL[indL]<-OM@q[indL[,c(1,2)]]*OM@sel[indL[,1:3]]*OM@E[indL[,c(1,2,4,5)]]
  
  #got to here! translate RFL (fishing mort by length to fishing mort by age)
  Ftrans<-array(0,c(nsim,nfleets,nyears,nages,nlen,nareas,npop))
  Find<-as.matrix(expand.grid(1:nsim,1:nfleets,1:nyears,1:nages,1:nlen,1:nareas,1:npop))
  
  Lind<-Find[,c(1,2,5,3,6)]
  Ftrans[Find]<-OM@iALK[Find[,c(1,7,3,4,5)]]*RFL[Lind]
  RF<-apply(Ftrans,c(1,7,4,3,6,2),sum) # s p a y r f
  
  maxRF<-apply(RF,c(1,2,4,5,6),max) 
  
  Rind<-as.matrix(expand.grid(1:nsim,1:npop,1:nages,1:nyears,1:nareas,1:nfleets))
  sel<-RF
  sel[Rind]<-sel[Rind]/maxRF[Rind[,c(1,2,4,5,6)]]
  sel<-sel[,,,nyears,,] # Take this from last year, in future simulations this may be by year so leave this code!
  ...
  for(m in 1:nsubyears){
    ...
    SPAYMR[,5]<-m
    SPMR[,3]<-m
    N[SPAYMR]<-surv[SPA]*OM@R0[SP]*OM@popdyn_Idist[SPMR]                         # Calculate initial spawning stock numbers
    SSB[SPAYMR]<-N[SPAYMR]*Wt_age[SPAY]*OM@mat[SPAY]                                     # Calculate spawning stock biomass
    
  }
  
  for(pp in 1:npop){
    
    SSBA[,pp,1]<-apply(N[,pp,,1,OM@Recsubyr[pp],]*array(OM@mat[,pp,,y]*Wt_age[,pp,,1],dim=c(nsim,nages,nareas)),1,sum)
  ...
  for(m in 1:nsubyears){
    ...
    C[SPAYMRF2]<-N[SPAYMR2]*(exp(Z[SPAYMR2])-1)*FM[SPAYMRF2]/Z[SPAYMR2]
 ...
 # sofind<-TEG(c(nsim,nfleets,nages,nareas,npop))
 
  for(y in 2:nyears){
    
    #cat(".")
    #sofindE<-cbind(sofind[,1:2],rep(y,nrow(sofind)),sofind[,4])
    #sof<-array(NA,c(nsim,nfleets,nages,nareas,npop))
    #sof[sofind]<-OM@E[sofindE]*sel[sofind[,c(1,5,3,4,2)]]*OM@q[sofind[,1:2]]*4
    #sof<-apply(sof,c(1,3,4,5),sum) # s a r p
    #sofind2<-TEG(dim(sof))
    #sofmax<-apply(sof,c(1,3,4),max)
    #sof<-sof/sofmax[sofind2[,c(1,3,4)]] # !!! spatial vulnerability (max 1 across areas or max 1 in each area?) This options makes it max 1 across all areas
    ...
    for(m in 1:nsubyears){
      ...
      #p<-1
      #N[sim,p,a,y-1,2,]
      #N[sim,p,a,y-1,2,]*exp(-Z[sim,p,a,y-1,2,])
      #(N[sim,p,a,y-1,2,]*exp(-Z[sim,p,a,y-1,2,]))%*%OM@mov[sim,p,a,m,,]
      
      if(m==1){
        N[,,,y,1,]<-N[,,,y-1,nsubyears,]*exp(-Z[,,,y-1,nsubyears,])
      }else{
      ...
      #VBA[SPAYMR]<-(NSN[SPAYMR]+SSN[SPAYMR])*Wt_age[SPAY]*sof[SARP]                          # Calculate actual vunerable biomass
      
      #FD[,,y,m,]<-(apply(VB[,,,y,m,,],c(1,5,4),sum)^array(OM@Spat_targ,c(nsim,nfleets,nareas)))/
        #array(apply(apply(VB[,,,y,m,,],c(1,5,4),sum)^array(OM@Spat_targ,c(nsim,nfleets,nareas)),1:2,mean),c(nsim,nfleets,nareas)) # distribute E x qs x sel by area according to spatial targetting parameters
      FM[SPAYMRF2]<-RF[SPAYRF2]
      Ftot<-apply(FM[,,,y,m,,],1:4,sum)
      Z[SPAYMR]<-Ftot[SPAR]+M[SPAY]/nsubyears
      ...
      for(pp in 1:npop){
        if(OM@Recsubyr[pp]==m){
          ...
          # age fish
          SSBA[,pp,y]<-apply(N[,pp,,y,m,]*array(Wt_age[,pp,,y]*OM@mat[,pp,,nyears],dim=c(nsim,nages,nareas)),1,sum)
          SSBdist<-apply(N[,pp,,y,m,]*array(Wt_age[,pp,,y]*OM@mat[,pp,,nyears],dim=c(nsim,nages,nareas)),c(1,3),sum)/SSBA[,pp,y]
         
          # Maturity is refreshed which is dumb: kind of removes the point in modelling movement of modelling mature and non mature fish separtely
          N[,pp,2:nages,y,m,]<-N[,pp,1:(nages-1),y,m,]
  ...
  #find appropriate spawning r and season for a spawning index
  recN<-apply(SSN,c(1,2,5,6),sum,na.rm=T)
  getspawnbyr<-expand.grid(1:nsim,1:npop,1:nareas)
  getspawnbyr<-as.matrix(cbind(getspawnbyr[,1:2],OM@Recsubyr[getspawnbyr[,2]],getspawnbyr[,3]))
  recN<-array(recN[getspawnbyr],c(nsim,npop,nareas))
  getspawnr<-array(NA,c(nsim,npop))
  for(ss in 1:nsim)for(pp in 1:npop)getspawnr[ss,pp]<-(1:nareas)[match(max(recN[ss,pp,]),recN[ss,pp,])]
  
  #mult<-nyears*npop
  #FIerr<-array(trlnorm(nsim*mult,1,rep(.Object@Iimp,mult)),c(nsim,npop,nyears)) 
  #FIsum<-apply(SSBA[,,1:nyears],1:2,mean,na.rm=T)
  #ind<-as.matrix(expand.grid(1:nsim,1:npop,1:nyears))
  #FIobst<-FIerr*(SSBA[,,1:nyears]/FIsum[ind[,1:2]])
  ...
  # define spawnr - ie find the major spawning areas in season 2
  # sum N * Wt * mat
  # write up the table below as it is written to the input file. 
  #
  #FIobst<-N[,pp,,y,m,]*array(Wt_age[,pp,,y]*OM@mat[,pp,,nyears]
  ...
  for(sim in 1:nsim){
    ...
    CALt<-CALm[sim,,,,,,,]
    CALsum<-ceiling(apply(CALt,3:7,sum)-0.5)
    #CALtot<-apply(CALsum)
    
    
    par(mfrow=c(1,2))
    plot(CALsum[1,1,2,1,]/max(CALsum[1,1,2,1,]))
    lines(CALsum[1,1,2,2,]/max(CALsum[1,1,2,2,]),col='red')
    plot(OM@sel[sim,1,])
    ...
    # PSAT tagging --
    nPSATs<-10000
    PSATdist<-apply(C[sim,,,,,,],c(1,4,5),sum,na.rm=T)^0.01
    ...
    for(pp in 1:npop){
      for(ss in 1:nsubyears){
        for(rr in 1:nareas){
          if(PSATdist[pp,ss,rr]>0){
            for(i in 1:PSATdist[pp,ss,rr]){
              j<-j+1
              SOO[j,]<-apply(C[sim,,,ceiling(nyears*0.7),ss,rr,],1,sum)/sum(C[sim,,,ceiling(nyears*0.7),ss,rr,])
              track[j,1]<-rr
              sy[j]<-ss
              ...
              for(rpt in 2:nT[j]){
                ...
                track[j,rpt]<-(1:nareas)[rmultinom(1,1,mov[pp,m,track[j,rpt-1],])==1] # 
                SOO[j,]<-SOO[j,]*apply(C[sim,,,ceiling(nyears*0.7),m,track[j,rpt],],1,sum)/sum(C[sim,,,ceiling(nyears*0.7),m,track[j,rpt],])
              } # track length
              SOO[j,]<-SOO[j,]/sum(SOO[j,])
              #if(1%in%SOO[j,]){
                for(rpt in 2:nT[j]){
                  #m<-RPTind[ss,rpt]
                  PSAT<-rbind(PSAT,c(pp,ss,2,track[j,(rpt-1):rpt]))
                }
              #}else{
               # for(rpt in 2:nT[j]){
                #  #m<-RPTind[ss,rpt]
    ...
    PSAT<-aggregate(rep(1,nrow(PSAT)),by=list(PSAT[,1],PSAT[,2],PSAT[,3],PSAT[,4],PSAT[,5]),sum)
    #PSAT2<-PSAT2[2:nrow(PSAT2),]
    
    write("# nPSAT, PSATs data of known stock of origin p s t fr tr N",datfile,1,append=T)
    write(nrow(PSAT),datfile,1,append=T)
    
    write("# PSAT data of known stock of origin p s t fr tr N",datfile,1,append=T)
    write(t(PSAT),datfile,6,append=T)
    
    write("# nPSAT2, PSATs data of unknown stock of origin s t fr tr SOO(npop)",datfile,1,append=T)
    write(1,datfile,1,append=T)
    #write(nrow(PSAT2),datfile,1,append=T)
    
    write("# PSAT2 data of unknown stock of origin s t fr tr SOO(npop)",datfile,1,append=T)
    write(t(PSAT2),datfile,4+npop,append=T)
    ...
    muSOO<-20  # mean number of observations at those points
    SOO<-apply(C[sim,,,1:nyears,,,],c(1,3,4,5),sum)
    #Csum<-apply(SOO,2:4,sum)
    rat<-mean(SOO)/muSOO
    SOO<-SOO/rat
    ind<-expand.grid(1:nyears,1:nsubyears,1:nareas)[sample(1:(nyears*nsubyears*nareas),NSOO),]
    ind<-as.matrix(cbind(rep(1:npop,rep=NSOO),ind[rep(1:NSOO,each=npop),]))
    SOOobs<-as.matrix(cbind(ind,SOO[ind]))
    SOOobs<-SOOobs[SOOobs[,5]>0,] # remove zeros 
    
    write("# nSOOobs, number of stock of origin observations p y s r N",datfile,1,append=T)
    write(nrow(SOOobs),datfile,1,append=T)
    
    write("# SOOobs, stock of origin observations p y s r N",datfile,1,append=T)
    write(t(SOOobs),datfile,5,append=T)
    ...
    mov<-array(NA,c(npop,nsubyears,nareas,nareas))
    #mov[as.matrix(PSAT[,c(1,2,4,5)])]<-1
    
    movind<-mov1<-c(1,1,1,1)
    mov<-OM@mov[sim,,2,,,]
    mov[mov>0]<-1
    notanarea<-apply(mov,c(1,2,4),sum)
    notanarea<-array(as.integer(notanarea>0),dim(notanarea))
    can<-apply(mov,c(1,4),sum)
    can<-array(as.integer(can>0),dim(can))
    
    ind<-TEG(dim(mov))
    mov[ind]<-mov[ind]*notanarea[ind[,c(1,2,3)]]
    
    for(pp in 1:npop){
      for(ss in 1:nsubyears){
        for(rr in 1:nareas){
          np<-sum(mov[pp,ss,rr,],na.rm=T)
          if(np>0){
            fR<-match(1,mov[pp,ss,rr,])
            mov1<-rbind(mov1,c(pp,ss,rr,fR))
            if(np>1){
              oR<-grep(1,mov[pp,ss,rr,])
              oR<-oR[oR!=fR]
              for(i in 1:length(oR)){
                movind<-rbind(movind,c(pp,ss,rr,oR[i])) 
              }   
            }  
          }  
        }
    ...
    if(movtype==1){ # if a gravity formulation these indices are for the to area that should be estimated by season
      ...
      rs<-indr[can2==1]
      ps<-indp[can2==1]
      movind<-cbind(rep(ps,each=nsubyears),rep(1:nsubyears,length(rs)),rep(rs,each=nsubyears),rep(999,length(rs)*nsubyears))
      
    }
    
    write("# nMP, number of estimated movement parameters",datfile,1,append=T)
    if(movtype==1)write(nrow(movind)+nsubyears*npop,datfile,1,append=T)
    ...
    write(t(movind),datfile,4,append=T)
    ...
    write(t(mov1),datfile,4,append=T)
    ...
    Fsum<-FM[sim,1,,,,,]
    maxv<-apply(OM@sel[sim,,],1,which.max) # get age at apical F (modal sel)
    ...
    if(complexF==0)lnFini<-log(Fsum[ind])
    if(complexF==1)lnFini<-log(Fsub[nind[,2:5]])
    
    write("# lnF_ini, initial values for log R0",datfile,1,append=T)
    ...
    write(tomt(OM@mov[sim,,nages,,,]),datfile,nareas,append=T) # Movement probabilities
    ...
    system(paste(OMDir,"M3.exe",sep="/"),wait=T,show.output.on.console = F) # run the exe
