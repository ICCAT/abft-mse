...
//                                   		           14th April 2021                          
//                                                                     
//                           		 ICCAT GBYP, Tom Carruthers tom@bluematterscience.com 
//
// ===========================================================================================================================================
//
//
...
//
// M3 is currently in an ongoing developmental (beta) stage. Any comments, bugs or otherwise can be forwarded to tom@bluematterscience.com. 
//
...
//                  Nick Duprey, Ana Parma, Enrique Rodriguez-Marin, Rebecca Rademmeyer
//          
...
// 2) Stock separation is not explicit in the data except for stock of origin, some PSAT (not PSAT2) and some conventional tags
// 3) Movement by subyear is the movement into an area in that subyear: models runs in the order: F(y-1) M(y-1), mov(y), F(y), M(y), mov(y+1),...
// 4) Recapture probabilities are 100% assigned to the release area in first time step after which they are affected by movement e.g. for 
//    a 5 area model (0,1,0,0,0) (0.05,0.7,0.1,0.1,0.5),...
// 5) First subyear of first year is for initialisation and there is no recorded fishing
//
//-- Change v7.0 -----------
// 
// * Added the option of conditioning with hyperstability / hyperdepletion assumed in all indices via beta parameter (for a robustness scenario)
// * Very minor changes to check indexing given reconditioning
//
//-- Change v6.3 -----------
// 
// Following review of Carmen Fernandez:
//
// * q estimation changed to MLE approach (Iobs, CPUEobs and SPprior now have an additional column)
// * code cleaned to remove previous optional features and initial values
//
//-- Change v6.2 ------------
//
...
DATA_SECTION
	...
	init_matrix CPUEobs(1,nCPUEobs,1,10);  // A table of CPUE index observations by year, subyear, area, fleet, index No. (nE), cv, wt, w (for q MLE calc)
	...
	init_matrix Iobs(1,nIobs,1,12);        // Observed relative index observations year, subyear, area, stock, index number (nI), type (1) biomass (2) SSB, index, fleet (if type=4), cv, wt, w (for q MLE calc)
	...
	init_matrix PSAT(1,nPSAT,1,8);         // PSAT release-recapture events from stock, age class, subyear, timestep, from-area, to-area, N, Frac, total from area
	...
	init_number BSfrac; ///(1,np,1,ns);    // Now just mean western fraction     // The predicted biomass of East Stock in West Area (1st row), West Stock in East area (2nd row)
	...
	init_matrix SpatPr(1,nSpatPr,1,10);    // Spatial Prior y, q, r, Ino, Index, CV wt lf lt w
	
	init_int nSpatFrac;                    // Number of spatial fraction priors
	init_matrix SpatFrac(1,nSpatFrac,1,8); // Spatial fraction pp, age class, start year, end year, r, logit fraction, CV, wt 
	...
	init_int ET_LHF;                       // Electronic tag likelihood function 1: multinomial 2: scaled multinomial (zero intercept)
	init_int LC_LHF;                       // Length Comp likelihood function 1: lognormal w const 2: lognormal no const
	init_number betad;                     // Hyperstability beta parameter used in robustness tests. <1 is hyper stable >1 is hyperdeplete (I=qB^beta)
...
PARAMETER_SECTION
	...
	init_bounded_vector lnR0(1,nSR,11,15.5,Phase1);                     // Unfished recruitment (period 1, period 2)
	...
	// A long list of dev vectors to address seasonal spatial stock trends (sorry - this is ugly and inelegant!)
	...
	objective_function_value objG;              // Global objective function value
        ...
        matrix SSBmu(1,np,1,3);                     // A matrix by stock storing total SSB observed, total SSB predicted, and number of observations
        ...
        vector WB(1,2);                             // West biomass in/out of the Western Area
	...
	END_CALCS
        ...
        number SpatFracPred;                           // Predicted spatial fraction
	...
	// -- Hyperstability --
	number beta; // Conversion to double for pow calc
...
FUNCTION calcObjective
  {
	...
	beta = betad;                           // Hack to get pow(..., beta) working
