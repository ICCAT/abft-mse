...
//                          Doug Butterworth, Haritz Arrizabalaga, 
//                Toshi Kitakado, David Die, Miguel Santos, Polina Levontin  
//          
//                                    Previous collaborators: 
//                 Joe Powers, Campbell Davies, Laurie Kell, Antonio Di Natale,
//                  Paul de Bruyn, Yukio Takeuchi, Sylvain Bonhommeau, Clay Porch 
//    ------------------------------------------------------------------------------------------
//
//
...
PARAMETER_SECTION
	...
	init_bounded_matrix movest_p1_a1(1,ns,1,nr-2,-8.,8.,Phase3);
	init_bounded_matrix movest_p2_a1(1,ns,1,nr-2,-8.,8.,Phase3);
	init_bounded_matrix movest_p1_a2(1,ns,1,nr-2,-8.,8.,Phase2);
	init_bounded_matrix movest_p2_a2(1,ns,1,nr-2,-8.,8.,Phase2);
	init_bounded_matrix movest_p1_a3(1,ns,1,nr-2,-8.,8.,Phase3);
	init_bounded_matrix movest_p2_a3(1,ns,1,nr-2,-8.,8.,Phase3);
	init_bounded_vector lnqE(1,nE,-10.,1.,Phase2);                 // q estimates for E fleets
	...
	END_CALCS
       ...
       // matrix RD(1,np,1,ny);                      // Recruitment deviations from historical average
...
FUNCTION calcMovement
  {
	...
	for(int ss=1;ss<=ns;ss++){               // Loop over subyears
	  ...
	  for(int fr =1;fr<=nr;fr++){  
	   ...
	   //for(int ac=1;ac<=nma;ac++){
	   //   movcalc(1,ss,ac,fr,1)=-10.;  // no movement to GOM
	   //   movcalc(2,ss,ac,fr,nr)=-10.; // no movement to Med
           //}
	...
	//cout<<movest_p1_a3<<endl;
	//cout<<movest_p2_a1<<endl;
	//exit(1);
...
FUNCTION calcF
  {
	...
	for(int pp=1;pp<=np;pp++){                     // Loop over stocks
	  ...
	  for(int yy=1;yy<=ny;yy++){                   // Loop over years
        ...
        //cout<<FAT(1)(30)(2)(2)<<endl;
	//cout<<"----"<<endl;
	//cout<<FAT(2)(38)(3)(2)<<endl;
	//exit(1);      
...
FUNCTION initModel
  {
	...
	for(int pp=1;pp<=np;pp++){
	  ...
	  //SSB0(pp)=R0(pp,1)*sum(elem_prod(surv(pp),Fec(pp)));     // Unfished Spawning Stock Biomass
	  //SSB0(pp)+=R0(pp,1)*Fec(pp,na)*surv(pp,na)*mfexp(-Ma(pp,na))/(1-mfexp(-Ma(pp,na))); // indefinite integral of surv added to get plus group SSB0
	  SSB0(pp)=R0(pp,1)*SSBpR(pp);
	}
	
	//cout<<mov(1)(2)(na)<<endl;
	//cout;
	...
	for(int yy=2;yy<=nHy;yy++){         // Loop over historical years
	  ...
	  for(int ss=1;ss<=ns;ss++){         // Loop over seasons
		...
		for(int rr=1;rr<=nr;rr++){// Loop over areas
		 ...
		 //   cout<<"objSRA = "<<objSRA<<" yy = "<<yy-1<<" ss = "<<ss<<" aa = "<<aa<<" rr = "<<rr<<endl;
		 //   cout<<"HCobs(y-1) = "<<HCobs(yy-1,ns,aa,rr)<<"  Ntemp = "<<Ntemp<<"  HCobs/Ntemp = "<<HCobs(yy-1,ns,aa,rr)/(Ntemp+tiny)<<"  pen = "<<pen<<endl;
		 //   cout<<"-----"<<endl;
		 // }
...
FUNCTION calcObjective
  {
	...
	/*
	for(int i=1;i<=nPSAT2;i++){ // Tags with certain stock of origin
...
FUNCTION simsam
  {
        ...
        //cout<<"R0 stock = "<<SRp<<endl;                    // Estimated R0 
        //cout<<"muR0 = "<<log(muR0)<<endl; 
