---
title: "ABT-MSE: Atlantic Bluefin Tuna Management Strategy Evaluation (v2.3)"
...
The Atlantic Bluefin Tuna Management Strategy Evaluation is funded by the ICCAT GBYP (coordinated by Paul de Bruyn <paul.debruyn@iccat.int>). The technical aspects of this modelling work are overseen by the GBYP Core Modelling Group: David Die, Clay Porch, Laurie Kell, Antonio Di Natale, Doug Butterworth, Harritz Arrizabalaga, Yukio Takeuchi, Sylvain Bonhommeau, Toshi Kitakado, Miguel Santos, Paul de Bruyn and Polina Levontin. Previous collaborators include Joe Powers and Campbell Davies. 
...
## New Additions to this Version (v2.3)
...
install.packages("C:/ABT-MSE/R_package/ABTMSE_2.3.0.tar.gz") 
```
...
An MSE design object *Design* was also loaded with loadABT(). *Design* is a list of the factors and their levels used in defining the reference set of operating models, e.g. [SCRS/2017/223](https://drive.google.com/open?id=1oJmXDD6tRKbEI4DwZnsLbFIAcq1EfiPt). This replicates the MSE design outlined in the [Trial Specifications document](https://drive.google.com/open?id=14j33JBhoep0TtvRj4pBkA8cIwM7XhGz3) .
...
You'll notice that some objects in the ABTMSE package have the extension *\_example*. These are stream-lined objects that are suitable for tutorials and demonstrations which involve less computational overhead (e.g. they require fewer simulations and require less system memory).
...
A related issue is fleet allocation of regional TACs that are calculated by MPs. Again we're going to leave allocation for a later section of this document as it is a bit complicated. The default will be used here which allocates regional TAC (e.g. West/East) in proportion to the fraction of catch by each fleet in that region over the last three years (2012-2014). 
...
A very large amount of information has been stored in the MSE object *MSE_example*. There are many ways to analyse these simulated data. 
...
While prioritization of management objectives has not yet been established in this early phase of the ABT MSE, a number of tentative performance metrics have been outlined in the [Trial Specifications document](https://drive.google.com/open?id=14j33JBhoep0TtvRj4pBkA8cIwM7XhGz3)
...
The estimation 'engine' behind the ABTMSE R framework is M3, a multi-stock population dynamics model written in ADMB. Your ABT-MSE folder contains a subfolder "Manuals\_and\_design_documents" that contains further information about M3 including model equations and software design documents (they can be found online [here](https://drive.google.com/open?id=1FnOXvaM9V71G3rO_jiQVVg3ljjNkmQcM)). 
...
An important axis of uncertainty in this MSE is future recruitment. This is described in the [Trial Specifications document](https://drive.google.com/open?id=14j33JBhoep0TtvRj4pBkA8cIwM7XhGz3) and the MSE *Design* object:
...
Additionally every time an MSE is run, the most current simulated dataset is passed to the global environment (called *dset*). It follows that if the MSE function crashes due to an error in an MP, it is easier to test the MP again using the offending simulated data and debug the issue. 
...
```{r Step_4_SP_test1a}
 ...
 SP_i7(x=1,dset=dset_example_West,startD=0.2)
...
We can extract the current index levels (years 2013 to 2015 are years 31 to 33) for each index from the observation model slot MPind that includes all the index data:

```{r MPdev2}
MPind<-Bad_Obs@MPind
IndE<-MPind[MPind$No==3 & MPind$Year%in%(31:33),] # get the Med Larval Survey
IndW<-MPind[MPind$No==7 & MPind$Year%in%(31:33),] # get the GOM Larval Survey
...
MP_E = function(x,dset,Targ=22.56,Delta=0.15,IndexNo=3,yrs4mean=4){
 ...
 curI = mean(dset$Iobs[x,IndexNo,datayrs]) # mean of last four years
...
```{r MPdevW}
MP_W = function(x,dset,Targ=o.42,Delta=0.15,IndexNo=7,yrs4mean=4){
...
We can see that both the alternative *MPareas* and the alternative fleet allocation tended to reduce expected long-term yield for the Eastern Stock for the UMSY and mean catch MPs. 
