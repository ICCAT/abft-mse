---
title: 'ABT-MSE: Atlantic Bluefin Tuna Management Strategy Evaluation (v5.2.2)'
...
* To allow for robustness OMs 32-33, Implementation model (class IE) 'Overages_20' and Observation model (class Obs) 'Unreported_20' can be specied in which fleets catch 6/5 of their allocation but report 5/6, respectively. 

* The MSE object now returns a slot Fleet_comp that includes the catch composition of each fleet. A 5d array (simulation, MP, fleet, stock, age) 

## Version 5.2.1

* Catchability increases to future CPUE indices can be specified using the qinc slot of the OM object (units are % annual increase and are applied to all simulations and CPUE indices identically)

* TAC allocation improved and now should exactly match TAC specifie. This is now accessible via MSE object slots MSE@TAC (recommended by the CMP) and MSE@TACtaken (the catch weight taken)

## Version 5.1.3

* Three additional OMs (#13-#15) were added that provide greater contrast in Western stock biomass. These are identical to OMs #4-#6 but were fitted with increased weight (precision) added to the western-only GOM_LAR_SUV index. 

## Version 5.1.2

* The new 12 reference OMs were added that span 3 recruitment scenarios, two M/maturity scenarios and two mixing scenarios

## Version 5.1.1 

* Line by line debug of OM translation code (bias correction for log normal recruitment deviations etc)
* 'Check Mode' was added to runMSE to test fidelity of ABTMSE R package historical numbers reconstruction with that of the M3 model. E.g. new('MSE',OM_1d, check=T)

## Version 5.0.1

* Overhaul of OMs and upgrade to the latest version of the M3 operating model (M3 v5.0: selectivity mirroring, size-specific indices, annual deviations from master index, etc)
* Dynamic B0 calculations now added to MSE object in slots dynB0 (projected) and dynB0h (historical)
* All existing performance metrics now phrase B relative to BMSY as SSB relative to SSBMSY_dynamic (where SSBMSY_dynamic is dynamic SSB0 * time invariant SSBMSY relative to SSB0)
...
```{r plotareas,echo=F,message=F,warning=F,error=F,fig.width=4, fig.height=3.5}
par(mar=rep(0.01,4))
areaplot(MSE_example)
```

The model is disaggregated into quarterly time blocks (ie Jan-Mar, Apr-Jun, Jul-Sept, Oct-Dec). This is primarily a constraint arising from how data have been reported historically.  
...
The MSE design (range of OMs) is described in the [report of the Jan 2019 bluefin MSE meeting](https://drive.google.com/open?id=10sWypo0nFdruIJ-6x1M_gTcB0AC6_5uJ)
...
All of the data that are used in the operational modelling are available [here](https://drive.google.com/open?id=18jqJXqwgDmnq48Ww6AskgS5J9J5T4exO)
...
install.packages("C:/ABT-MSE/ABTMSE_5.2.2.tar.gz") 
```
...
An MSE design object *Design* was also loaded with loadABT(). *Design* is a list of the factors and their levels used in defining the reference set of operating models, e.g. the MSE design outlined in the [Trial Specifications document](https://drive.google.com/open?id=16YIFO0avqFjsJo3iLjnfhfkyIv6k-awR) (note this is currently being updated).

Here is a figure showing how the reference OM numbering relates to the three factors (recruitment, spawning fraction / natural mortality rate M, stock mixing):

<img src="Ref_OMs.jpg" width="280">
...
This design grid explains the nomenclature of the various operating models (class OM) and their input definition files (class OMI) which are numbered from 1 to 12 corresponding with the rows of this design matrix:
...
<img src="Rob_OMs.jpg" width="800">

To run robustness OMs 32 and 33 you just run OM_1 and OM_2 with the 'Overages_20' implementation error model and the 'Unreported_20' Observation model (6/5 of the TAC taken, 5/6 reported) 
...
A related issue is fleet allocation of regional TACs that are calculated by MPs. Again we're going to leave allocation for a later section of this document as it is a bit complicated, for now all you need to know is that it mirrors recent spatial fleet allocations in the East and West management areas (ie it reflects the status quo by default). 
...
A search for operating models reveals `r floor((length(avail('OM'))-1)/2)` reference OMs and 1 example:
...
In the case of the *POF* function three slots of an MSE object are used.
  ...
  MSE@F_FMSY is the slot containing fishing rate relative to FMSY (these are actually harvest rate U, relative to U at MSY)
...
out<-M3read("C:/Users/tcar_/Dropbox/abft-mse/M3") # get the raw M3 model fit 
load("C:/Users/tcar_/Dropbox/abft-mse/M3/OMI")    # load the corresponding OMI object in the same folder
