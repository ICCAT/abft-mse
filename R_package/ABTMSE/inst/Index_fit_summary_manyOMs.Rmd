...
for(i in 1:n_ind){
  ...
  layout(t(matrix(c(rep(c(1,2),each=4),rep(c(3,4,5,6),each=2),6+(1:nOMs)),nrow=8)))
...
```{r statmat_SD, fig.width=8, fig.height=24,echo=FALSE,warning=F,error=F,message=F}
  ...
  colnams<-c("OM","R","P","M","S","L",sapply(1:n_ind,replacewspace,y=Ilabs))
