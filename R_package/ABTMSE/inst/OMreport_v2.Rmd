...
allSSB<-cbind(apply(out$hSSB[,2:nHy,],1:2,mean),apply(out$SSB,1:2,mean))# some some reason past understanding the model sends the wrong numbers for the first year in SSB
ayv<-(OMI@Hyears[1]+1):OMI@years[2] # some some reason past understanding the model sends the wrong numbers for the first year in SSB
...
Figure 6. Model predicted spawning stock biomass and recruitment. h is the assumed value of steepness, lnR0 is the log unfished recruitment. The red point is the most recent SSB-Recruitment estimate. 
...
```{r estimatedsels, fig.width=6.5, fig.height=4,echo=FALSE}
  ...
  nf<-out$nf
  nl<-out$nl
  fcols<-rep(c("black","orange","#99999980","#ff000080","#00ff0080","#0000ff80","#80802080","#ff00ff80","#00ffff80"),2)
...
if(!is.null(OMI@SpatPr)){
   ...
   #Bsum<-apply(B[,,,5:out$na,],c(2,3,5),sum) # SSB by year and area in spawning season
