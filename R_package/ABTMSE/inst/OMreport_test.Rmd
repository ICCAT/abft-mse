...
res2<-doMSY(out,OMI,dynB0,refyr,res)
...
Figure 9a. The size selectivity of the various fishing fleets included in the operating model. 
...
Figure 9b. The size selectivity of the various fishing fleets included in the operating model. 
...
```{r fit to agg length observations, fig.width=7.5, fig.height=9,echo=FALSE}
  ...
  # All spatio temporal strata
  #tiny <- 0.0000000001
  #dim(out$CLtotpred) # 52  4  7 18 15
  #CF_CLpred <- aperm( apply(out$CLtotpred, 1:4, function(x){x/ (sum(x)+tiny) }), perm = c(2:5,1) )
  #CF_CLpred_aggreg <- apply(CF_CLpred, c(1,4,5), sum )
  #CFauxtot<-array(pmax(apply(CF_CLpred_aggreg,1:2,sum),0.00001),dim(CF_CLpred_aggreg))
  #CF_CLpred_aggreg <- CF_CLpred_aggreg/CFauxtot
  #CF_CLpred_aggrega <-apply(CF_CLpred_aggreg,2:3,sum,na.rm=T)/apply(CF_CLpred_aggreg,2,sum,na.rm=T)
  #CF_CLpred_aggrega10 <-apply(CF_CLpred_aggreg[ind,,],2:3,sum,na.rm=T)/apply(CF_CLpred_aggreg[ind,,],2,sum,na.rm=T)
  ...
  dmat<-array(c((1:8)*3-2,(1:8)*3-2,(1:8)*3-1,(1:8)*3),c(8,4))
  layout(dmat)
  ...
  for(ff in 1:8){
    ...
    if(all(is.na(CLoa10[ff,]))){
      
      plot(1,col='white',axes=F,main="",xlab="",ylab="")
      
    }else{
      
      ylim=c(0,max(CLoa10[ff,],CLpa10[ff,]))
      barplotT(out$ml,CLoa10[ff,],ylim=ylim)
 
      lines(out$ml,CLpa10[ff,],col="#0000ff90",lwd=2)
      lines(out$ml,CLpaZ10[ff,],col="green",lwd=2)
...
Figure 11. Length composition residuals. The left-hand panels show residuals over time. Orange circles are positive residuals (observed proportion is higher than expected), blue circles are negative residuals. The area of the circle is proportional to the absolute residuals error (i.e. if the observed proportion of a length class is 0.5 and the model predicted fraction is 0.4, this is represented by a circle ten time larger than if the observed proportion was 0.05 and the predicted fraction is 0.04). This keeps the scaling of the left-hand plots the same as the right-hand plots that show the aggregate residual error over all years and (right mosts panels) the period since 2007. The blue line should match the orange bars when model predictions fit observations. The green line is the predicted length composition across all length classes for all time-area strata where there is at least one length observation (even though observations of zero proportions do not enter the likelihood function). The green lines may not match the observations (orange bars) or the predicted positive length proportions (blue lines) if length observations (zero proportions) are not missing at random.  
...
```{r fit to agg length observations2, fig.width=7.5, fig.height=9,echo=FALSE}
  ...
  for(ff in 9:nf){
    ...
    if(ff==9)legend('topright',legend="All years",bty='n') 
    ...
    if(all(is.na(CLoa10[ff,]))){
      ...
      if(ff==9)legend('topright',legend=paste(OMI@years[2]-9,"-",OMI@years[2]),bty='n')
    }
...
```{r Res, fig.width=7.5, fig.height=9,echo=FALSE}
  ...
  for(f in 1:out$nf){
    ...
    barplot(apply(eps[,f,],2,sum,na.rm=T))
    legend('topleft',legend=OMI@Fleets$name[f],bty='n')
...
Figure 11b. Length composition residuals 
...
Figure 17b. Predicted spatial distribution of Eastern stock biomass by year and season. The red line is the fraction (%) of stock biomass found in each area.  
...
for(rr in 1:nr){
  for(ss in 1:ns){
    ...
    abline(h=ys,col="light grey")
    abline(h=c(50,100),col="black")
    lines(yrs,Bf,col="blue")
    points(yrs,Bf,col="blue",pch=19,cex=0.8)
    if(rr==nr)axis(1,xs,xs)
    if(rr<nr)axis(1,xs,NA)
    if(ss==1)axis(2,ys,ys)
    if(ss<(ns+1))axis(2,ys,NA)
...
Figure 17c. Predicted spatial distribution of Western stock biomass by year and season. The red line is the fraction (%) of stock biomass found in each area.  
...
if(!is.null(OMI@SpatPr)){
   ...
   ni<-max(OMI@SpatPr[,4]) 
   par(mfrow=c(ceiling(ni/2),2),mai=c(0.3,0.3,0.01,0.01),omi=c(0.4,0.4,0.01,0.01))
