...
for(i in 1:nOMs){
  ...
  res<-MSYMLE(out,OMI)
  res2<-doMSY(out,OMI,dynB0[i,,],refyr,res)
  ...
  Sres1<-rbind(Sres1,c(OM=OMlab[i],Code=OMnames[i],res2[1,]))
  Sres2<-rbind(Sres2,c(OM=OMlab[i],Code=OMnames[i],res2[2,]))
...
```{r SSBa_overlap, results='asis',echo=FALSE,fig.width=9,fig.height=6}
  ...
  for(aa in 2:1){
    ...
    if(aa==1) legend('bottom',legend=c("Rec 1","Rec 2","Mat/M A","Mat/M B","Mix I","Mix II"),lty=c(1,1,1,2,1,1),lwd=c(1,1,1,1,1,2),col=c("black","red",rep('black',4)),bty='n')
...
for(i in 1:nOMs){
  ...
  plot(yrs,SSBstore[i,1,]/1000,col="#ff000098",type="l",ylim=c(0,SSBEmax),ylab="",xlab="",yaxs="i")
  #axis(1)
  #axis(2,col="red")
