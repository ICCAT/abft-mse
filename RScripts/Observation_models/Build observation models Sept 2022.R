...
# these are rescaled in /BFT MSE/Communications/Index rescaling
CPUEdat<-read.csv(file=paste(getwd(),"/data/ICCAT_2022_2/CPUE_indices compiled_OM_7Sep2022_wNA_RESCALED.csv",sep=""))[,1:9] # !TSD Table 2.1
Iobs<-read.csv(paste(getwd(),"/data/ICCAT_2022_2/FI_indices_compiled_OM_7Sep2022_wNA_RESCALED.csv",sep=""))
...
min<-aggregate(Indstats$Index,by=list(Indstats$Name),min,na.rm=T)
max<-aggregate(Indstats$Index,by=list(Indstats$Name),max,na.rm=T)
mu<-aggregate(Indstats$Index,by=list(Indstats$Name),mean,na.rm=T)
med<-aggregate(Indstats$Index,by=list(Indstats$Name),quantile,p=0.5,na.rm=T)
minyr<-aggregate(Indstats$Year+1964,by=list(Indstats$Name),min,na.rm=T)
maxyr<-aggregate(Indstats$Year+1964,by=list(Indstats$Name),max,na.rm=T)
