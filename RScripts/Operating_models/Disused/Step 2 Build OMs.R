...
saveRDS(Design,file=paste0(getwd(),"/Objects/OMs/Design.Rdata"))
saveRDS(Design,file=paste0(getwd(),"/R_package/ABTMSE/data/Design.Rdata"))
...
if(OMIbuild){
  ...
  for(i in 1:(nOMs/2)){
    ...
    OMI@SOOobs[OMI@SOOobs[,8]==1,7]<-10
  ...
  for(i in 1:length(fromdirs)){
    ...
    # downweight all microchemistry data
   
    # --- Prior weight on fraction of stock in opposing area   -----------
    #out<-M3read(OMDir=paste0(getwd(),"/Objects/OMs/",fromdirs[i],"/"))
    #mov<-out$mov
    #nages<-out$na
    #npop<-out$np
    #nsubyears<-out$ns
    ...
    #BSfrac[1,]<-apply(stemp[1,,1:3],1,sum)/apply(stemp[1,,],1,sum) # East stock in the western area
    #BSfrac[2,]<-apply(stemp[2,,4:nareas],1,sum)/apply(stemp[2,,],1,sum) # West tsock in the eastern area
    #BSfrac[1,]<-BSfrac[1,]/2 # half eastern stock in western area
    #BSfrac[2,]<-0.001 # only 0.1% western stock in easter area (fixed anyway by movement exclusion below)
    #OMI@BSfracCV<-0.1
    #OMI@BSfrac<-BSfrac
    #OMI@LHw[16]<-10

    # --- Movement exclusion -----------------------------
   # MEM<-array(NA,c(OMI@np,OMI@nma,  OMI@ns, OMI@nr,    OMI@nr))
  #  ER<-(-20)
    #No western fish in the GOM in Quarter 3
   # GOM<-match('GOM',OMI@areanams)
    #MEM[2,,3,,GOM]<-ER
    #No Western fish in eastern areas
    #MED<-match('MED',OMI@areanams)
    #MEM[2,,,,MED]<-ER
    #SATL<-match('SATL',OMI@areanams)
    #MEM[2,,,,SATL]<-ER
    #NATL<-match('NATL',OMI@areanams)
    #MEM[2,,,,NATL]<-ER
    #EATL<-match('EATL',OMI@areanams)
    #MEM[2,,,,EATL]<-ER
...
if(reportbuild){
  ...
  # Individual reports
  Design=readRDS(file=paste0(getwd(),"/Objects/OMs/Design.Rdata"))
  nOMs<-12
  ...
  render(input=paste0(wd,"/R_package/ABTMSE/inst/OMsummary.Rmd"), output_file=paste(dir,"/Summary_Report_2019.html",sep=""))
  ...
  load(designdir)
  OMcodes<-apply(Design_Ref,1,FUN=function(x)paste(x,collapse="-"))
  OMnos<-(1:length(OMcodes))[!grepl("3",OMcodes)&!grepl("C",OMcodes)]           # The numbering of the reference OMs 1-
  OMdirs<-paste0(wd,"/Objects/OMs/",OMnos)
  nOMs<-length(OMdirs)
  OMnames<-OMcodes[OMnos]
  ...
  files<-paste0(dir,c("/Summary_Report_As.html","/Summary_Report_Bs.html","/OM_LH_comp.html"))
