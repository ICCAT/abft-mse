...
wd<-"C:/Users/tcarruth/Dropbox/abft-mse"
...
Design_Ref<-rbind(expand.grid(all_levs),data.frame(Var1=paste0(1:3,"w"),Var2=rep("B",3),Var3=rep("I",3)))                                              # The full design grid (all combinations) of the various factors and their levels
...
if(OMIbuild){
  ...
  # Now the new GOM_Ind precise runs
  ...
  for(i in 1:length(fromdirs2)){

    print(paste(todirs2[i],"/",nOMs))
    load(file=paste(getwd(),"/Objects/OMs/",fromdirs2[i],"/OMI",sep=""))             # OMI loaded
    OMcode<-OMcodes[todirs2[i]]
    OMI@Name<-paste0(todirs2[i],"/",nOMs," : ",OMcode)
    OMI@IobsCV[5]<-0.15

    OMfolder<-paste(getwd(),"/Objects/OMs/",todirs2[i],sep="")
    OMI@OMfactors<-as.list(LNames_Ref[todirs2[i],])
...
if(runM3OM){
  ...
  foldernos<-c(fromdirs,todirs,todirs2)
...
if(OMbuild){
  ...
  folderno1<-(1:nOMs)[grepl("1",Design$Design_Ref[,1])] # foldernos<-c(1,6,8,9,13,17,21,25,29,33) foldernos<-c(3,11,16,20,21,23,27,31,33,35)
  folderno2<-(1:nOMs)[grepl("2",Design$Design_Ref[,1])] # foldernos<-c(1,6,8,9,13,17,21,25,29,33) foldernos<-c(3,11,16,20,21,23,27,31,33,35)
  ...
  for(i in 1:length(reffolders)){
      ...
      if((!file.exists(OMfolder)&overwrite==F)|overwrite){
          ...
          j<-as.numeric(strsplit(as.character(Design$Design_Ref[i,1]),"w")[[1]])
          print(j)
...
if(reportbuild){
  ...
  for(ij in 1:length(OMdirs)){
    ...
    render(input="C:/Users/tcarruth/Dropbox/abft-mse/R_package/ABTMSE/inst/OMreport.Rmd", output_file=paste(input_dir,"/Report_",lab,".html",sep=""))
  }
  ...
  OMnos<-(1:length(OMcodes))[grepl("1-",OMcodes)|grepl("2-",OMcodes)|grepl("1w-",OMcodes)|grepl("2w-",OMcodes)]           # The numbering of the reference OMs 1-
  OMdirs<-paste0(wd,"/Objects/OMs/",OMnos)
  nOMs<-length(OMdirs)
  OMnames<-OMcodes[OMnos]
  introtext<-"This is a report of all 2019 reference grid operating models (MPD estimates). Note that operating models with recruitment level 1 and 3 are identical in their historical recruitment and are therefore identical in their model fitting. Only levels 1 and 2 are presented here. Operating models labelled with a 'w' (e.g. 2w-B-I) include greater precision in the GOM_LAR_SUV index (CV of 15%) to create scenarios with greater contrast in current Western stock status."
  render(input=paste0(wd,"/R_package/ABTMSE/inst/OMsummary_Gomind.Rmd"), output_file=paste(dir,"/Summary_Report_2019.html",sep=""))
