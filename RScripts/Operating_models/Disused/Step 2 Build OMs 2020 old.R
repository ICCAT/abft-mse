...
OMIt@LHw <-   c(1/50,  1,       2,          0.5,         1,     5,       0,       1,        1,     1,      1,      1,      0,         1,        1,         1,         1,       1)
...
for(i in 1:nOMs)pin_from_par(dirs[i])
...
# Do the index fitting reports again
...
nOMs<-length(fitdirs)
dirs<-paste0(OMdir,fitdirs)
outfile<-paste0(OMdir,"/Range summary.html")
render(input=paste0(getwd(),"/R_package/ABTMSE/inst/OM_ranges.Rmd"),
       output_file=outfile)
