...
CCV<- 0.02*(Base@ns*Base@nr/2)^0.5 # per fleet obs annual obs error is ~ 0.02 assuming half coverage
...
Ilencat<-matrix(c(2,4,4,5,1,5,6,15,7,15,rep(0,2*(Base@nCPUEq-5))),nrow=2) # make this generic nfleets long
Base@Ilencat<-Ilencat
#ind<-Base@CPUEobs[match(1:max(Base@CPUEobs[,4]),Base@CPUEobs[,4]),7]
#ind[ind==0]<-NA
#rbind(Base@CPUEnames,ind,Ilencat,Base@mulen[Ilencat[1,ind]],Base@mulen[Ilencat[2,ind]]) # lt nos
...
Base@Iobs[match(1:max(Base@Iobs[,5]),Base@Iobs[,5]),]
...
Base@movCV     =3                 # was 1        # Prior precision of deviations from homogeneous movement
Base@selCV     =1.5                        # Prior precision of selectivity parameters
...
Base@LHw <-  c(rep(StWT,4), c(SOOUWT,StWT,StWT),  rep(priorDWT,10))/
            c(Base@nCobs, Base@nCPUEobs, Base@nIobs, Base@nCLobs, Base@nSOOobs, Base@nPSAT, 0,       Base@ny*Base@np,

              #              9 mov,                            10 sel,           11 SRA,  12 SSB, 13 SSBinc,
              Base@ns*(Base@nr-2)*Base@nma*Base@np, sum(Base@seltype), nrow(Base@HCobs),  0,      0,

               #    14 Fmod,  15 R0diff, 16 BSfrac, 17 MICV
              Base@nr*Base@ns, Base@np,    0,         Base@nr*Base@ny*Base@ns)

Base@LHw[Base@LHw==Inf]<-0




# MI invariant specifications
Base@Phases<-as.integer(rep(1,4))#as.integer(c(1,2,3,4))
Base@MICV<-1.5
