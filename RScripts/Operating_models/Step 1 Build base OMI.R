...
CCV<- 0.02 #0.005*(Base@ns*Base@nr/2)^0.5 # per fleet obs annual obs error is ~ 0.02 assuming half coverage
...
Ilencat<-matrix(c(2, 4, 7,  2, 8,  6,rep(0,Base@nCPUEq-6),
                  5, 5, 15, 5, 15, 15,rep(0,Base@nCPUEq-6)),nrow=2,byrow=T) # make this generic nfleets long
...
# aggregate(Base@CPUEobs[,7],by=list(Base@CPUEobs[,4]),mean)
...
Base@Fleets$UB[match("RRUSAFS",Base@Fleets$name)]<-150
Base@Fleets$LB[match("RRUSAFB",Base@Fleets$name)]<-150
...
source("Rscripts/Data processing/FI indices 2019 lt.r") # returns Iobs and Inames
...
source("Rscripts/Data processing/Spatial_Priors.r") # returns SpatPr
...
Base@BSfracCV  = 0.02
Base@nLHw      = as.integer(18)             # number of likelihood components that may be weighted
...
#              1 catch,   2 cpue,   3 FIindex,  4 Lcomp,     5 SOO,       6 PSAT,    7 PSAT2,  8 RecDev,
Base@LHw <-   c(1/1000,   1,        1,          1/10000,     1/10,        1/10,         0,     1/10,

           #    9 mov,    10 sel, 11 SRA,  12 SSB, 13 SSBinc,
                1/10,     1/10,   1/10,       0,      0,

          #    14 Fmod,  15 R0diff, 16 BSfrac, 17 MICV,  18 SpatPr
...
Base@CLobs[,7]<-1
Base@SOOobs[,9]<-1
Base@SOOobs[Base@SOOobs[,8]==1,9]<-1/6
