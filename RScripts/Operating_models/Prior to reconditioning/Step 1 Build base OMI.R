...
Base@years<-years<-c(1965,2016)
Base@Hyears<-Hyears<-c(1864,1964)
...
source("Rscripts/Data processing/Master index 2.r") # returns list MI: MI[[1]] GLM from task II CPUE MI[[2]] SS assessment abundance x etagging  movement MI[[3]] flat, constant
...
source("Rscripts/Data processing/Fishery indices Nov 2019.r") # returns CPUEqvec and CPUEobs and Ilencat in the right format
...
#                                  # this is CAN_ACO_SUV
Ilencat<-matrix(c(2, 4, 7,  2, 8,  6,rep(0,Base@nCPUEq-6),
                  4, 5, 15, 5, 15, 15,rep(0,Base@nCPUEq-6)),nrow=2,byrow=T) # make this generic nfleets long
Base@Ilencat<-Ilencat
#                   lencat 1        lencat 2       lencat 3     lencat 4   lencat 5
lencatfleets<-c("US_RR_66_114","US_RR_115_144","US_RR_177","US_RR_145","US_RR_195")
for(l in 1:length(lencatfleets)) Base@CPUEobs[Base@CPUEobs[,4]==match(lencatfleets[l],Base@CPUEnames),7]<-l
# cbind(Base@CPUEnames,aggregate(Base@CPUEobs[,7],by=list(Base@CPUEobs[,4]),mean))
# cbind(Base@CPUEnames,aggregate(Base@CPUEobs[,5],by=list(Base@CPUEobs[,4]),mean))
if(sum(is.na(Base@CPUEobs[,4]))>0)message("!!!Missing length categories detected!!!")


# --- (h) Calculate length sample data ------

source("Rscripts/Data processing/Length observations Dec 2019.r") # returns CLobs
...
Base@Fleets$UB[Base@Fleets$UB<Base@nl]<-Base@Fleets$UB[Base@Fleets$UB<Base@nl]
...
source("Rscripts/Data processing/FI indices Nov 2019.r") # returns Iobs and Inames
...
Base@macat<-matrix(c(1,4,5,8,9,Base@na),nrow=2)
...
Base@BSfrac<-array(0.1,c(1,1))

# --- Spatial seasonal priors -------------------------------------------------------

source("Rscripts/Data processing/Spatial_Priors Nov 2019.r") # returns SpatPr
...
wt<-rep(1,Base@nCLobs) #1/(Base@CLCV_num/Base@CLobs[,6]) # sd is sqrt of denominator
...
Base@FCV       = 1                                  # was 0.4        # Prior precision of season-area deviations around mean F
Base@movCV     = 4                                  # was 1        # Prior precision of deviations from homogeneous movement
Base@selCV     = 4                                  # Prior precision of selectivity parameters
Base@SSBincCV  = 0.01                               # Prior precision of SSB increase ratio
Base@R0diffCV  = 0.4                                # was 0.5  # Prior on diferrence in early and late R0 estimation (where applicable)
Base@BSfracCV  = 0.02
...
Base@LHw <-   c(1/100,   1,      2,         0.05,      1,     5,       0,        1,        1,     1,      1,      0,      0,         1,        1,         0,         1,       1)
Base@nLHw<-as.integer(18)
Base@Cobs[,7]<-1
Base@CPUEobs[,9]<-1
Base@CLobs[,7]<-1
Base@SOOobs[,9]<-1
Base@Iobs[,10]<-1
Base@Iobs[Base@Iobs[,5]==match("GOM_LAR_SUV",Base@Inames),10]<-10   # upweight gom lar suv
Base@Iobs[Base@Iobs[,5]==match("MED_LAR_SUV",Base@Inames),10]<-5   # upweight med lar suv
#Base@SOOobs[Base@SOOobs[,8]==1,9]<-1/4                                # downweight microchem
...
Base@LC_LHF<-2
Base@SpatPr[,6]<-0.05
#                       s, ac, sy, en, r,   logit(frac),    CV,  wt
Base@SpatFrac<-matrix(c(1, 3,  48, 52, 4, log(0.2/(1-0.2)), 0.1, 0),nrow=1)

Base<-Rec_Ref(Base,lev=2)
