...
HCobs_tab_BRA<-read.csv(paste(getwd(),"/data/ICCAT_2019_3/JPN_offBRA_1957_1964.csv",sep=""))
idcat<-sapply(1:nrow(HCobs_tab_BRA),function(x,dat)paste0(dat[x,1:3],collapse="-"),dat=HCobs_tab_BRA)
idfrom<-sapply(1:nrow(HCobs_tab_BRA),function(x,dat)paste0(c(dat[x,1:2],2,dat[x,3]),collapse="-"),dat=HCobs_tab_BRA)
idto<-sapply(1:nrow(HCobs_tab_BRA),function(x,dat)paste0(c(dat[x,1:2],4,dat[x,3]),collapse="-"),dat=HCobs_tab_BRA)
idref<-sapply(1:nrow(HCobs_tab),function(x,dat)paste0(dat[x,1:4],collapse="-"),dat=HCobs_tab)

locfrom<-match(idfrom,idref)
locto<-match(idto,idref)

check<-sum(HCobs_tab[locfrom,5]-HCobs_tab_BRA[,4]<0)==0
HCobs_tab[locto,5]<-HCobs_tab[locto,5]+HCobs_tab_BRA[,4]
HCobs_tab[locfrom,5]<-HCobs_tab[locfrom,5]-HCobs_tab_BRA[,4]
...
if(diag){  # some diagnostics
  ...
  HCobs_old<-readRDS(file=paste(getwd(),"/Data/Processed/Conditioning/HCobs_BRA.rda",sep=""));
