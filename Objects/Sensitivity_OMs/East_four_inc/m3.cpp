...
model_data::model_data(int argc,char * argv[]) : ad_comm(argc,argv)
{
  ...
  CPUEobs.allocate(1,nCPUEobs,1,7,"CPUEobs");
  nE.allocate("nE");
  ...
  Ilencat.allocate(1,5,1,np,"Ilencat");
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  lnmuRT.allocate(11.5,16.5,1,"lnmuRT");
  lnPrat.allocate(1,"lnPrat");
  ...
  lnqE.allocate(1,nE,-10.,1.,1,"lnqE");
  ...
  VBi.allocate(1,ny,1,ns,1,nr,1,nf,"VBi");
    ...
    VBi.initialize();
  #endif
  VBind.allocate(1,ny,1,ns,1,nr,1,nf,"VBind");
  #ifndef NO_AD_INITIALIZE
    VBind.initialize();
  #endif
  ...
  CLprop.allocate(1,nCLobs,"CLprop");
    ...
    CLprop.initialize();
  #endif
  CLsd.allocate(1,nf,"CLsd");
  #ifndef NO_AD_INITIALIZE
    CLsd.initialize();
  #endif
  CLn.allocate(1,nf,"CLn");
  #ifndef NO_AD_INITIALIZE
    CLn.initialize();
  #endif
  CLvar.allocate(1,nf,"CLvar");
  #ifndef NO_AD_INITIALIZE
    CLvar.initialize();
  #endif
...
{
  {
	...
	for(int mm=1;mm<=nMP;mm++){
	  LHtemp=dnorm(movest(mm),0.,1.);        // Weak(ish) prior 
          objmov+=LHtemp*LHw(9);                  // Weighted likelihood contribution
	  objG+=LHtemp*LHw(9);                    // Weighted likelihood contribution
	}
	// -- Selectivity parameters ---
	for(int i=1;i<=nsel;i++){ 
	  objsel+=dnorm(selpar(i),0,0.9)*LHw(10);  
	  objG+=dnorm(selpar(i),0,0.9)*LHw(10);   // Prior on selectivity to add numerical stability
	}
	objG+=objSRA*LHw(11);                      // Add the posfun penalty for SRA harvest rates over 90%
	for(int pp=1;pp<=np;pp++){
	  switch(SSBfit){
	    case 1:  // Biomass
	      objSSB+=dnorm(log(SSBnow(pp)+tiny),log(SSBprior(pp)+tiny),SSBCV)*LHw(12);   // Predicted index
	      break;
	    case 2:  // SSB
	      objSSB+=dnorm(log(SSB0(pp)+tiny),log(SSBprior(pp)+tiny),SSBCV)*LHw(12);  // Predicted index normalized to mean 1
	      break;
	   }
	}
	objG+=objSSB*LHw(12);
	for(int ll=1;ll<=ns*nr;ll++){
	  objFmod+=dnorm(Fmod(ll),0,0.1);
	}
	objG+=objFmod;
	LHtemp=0.;
	CPUEtemp=0.; 
	//for(int yy=1;yy<=ny;yy++){	  
	for(int pp=1;pp<=np;pp++){                // Stocks
	  for(int aa=1; aa<=na; aa++){         // loop over movement age classes
	     for(int rr=5;rr<=nr;rr++){ 		          
	      LHtemp+=N(pp)(ny-9)(pp)(aa)(rr)*Fec(pp,aa);        // Stock numbers
	      CPUEtemp+=N(pp)(ny)(pp)(aa)(rr)*Fec(pp,aa); 
            }  
	  }
        } 
	//}
	objG+=dnorm(log(CPUEtemp/LHtemp), 1.2,0.01)*1000;
	objRat=dnorm(log(SSB0(1)/SSB0(2)),2.303,0.15);
	//objG+=objRat*50;
