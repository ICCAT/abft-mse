...
All of the data that are used in the operational modelling are available [here](https://github.com/ICCAT/abft-mse/tree/master/data/)
...
As you can see the slot *MPind* is quite large.
...
This code will test three management systems. One where UMSY is used in both the West and the East, a second where a stock reduction analysis DBSRA is used in the West and the East and a third management system where UMSY is used in the West and DBSRA is used in the East. The principal motivation for choosing these MPs in this tutorial is that they run quickly. 
...
You may have noticed that while we specified only three management systems (three pairs of East/West MPs) in fact, four were tested and that the first, labelled *ZeroC (West) and ZeroC (East)* we did not specify. 
...
This convention allows statistics to be generated for any metric. The Tplot function of the [quick-start section](#QS_Tplot) just plotted point values that were the mean values of the MPs. Lets calculate some other quantities for long term yield (*Y30*) and Final depletion (*D30*):

```{r qs_PMs_det, fig.width=3, fig.height=3.5}
Y30arr<-Y30(MSE_example)
D30arr<-D30(MSE_example)
apply(Y30arr,1,quantile,c(0.025,0.5,0.975)) # median and percentiles by MP (column)
plot(Y30arr[2,],D30arr[2,],xlab="Yield",ylab="Depletion",col="#0000ff95",pch=19) # plot of individual simulations
```
...
Other PMs are quite similar. Here is *Y10* (mean yield over the first 10 years of a projection):

```{r Step_2_examine_Y10}
?Y10
Y10
```

*Y10* is only concerned with averaging catches which are stored in the slot \*MSE@C\*. Again the apply() function is used to calculate mean catches by MP and simulation. 
...
This model is fitted to data to generate credible operating model scenarios for ABT population and fishery dynamics. The M3 model requires fishery, tagging and biological data formatted in specific way (for example catches in a table where each row is a catch record and the columns represent years, subyears, areas and fleets).

To make it easier for other users to modify the operating model data and assumptions, a new class of data object was designed. Named *OMI* (for Operating Model Input), this object class contains all the data in the correct format for input to the M3 model. This organisation of data allows for rapid changes may be made to an object of class OMI and then the M3 model run, with the minimum of code. 
...
```{r Step_3_OMI_6_interrogation2}
dim(OMI_6@mat)
```

The current maturity schedules are time-invariant (the same among model years) and quite different among the stocks:

```{r Step_3_OMI_6_old_mat,fig.width=4, fig.height=4}
matplot(t(OMI_6@mat[,,55]),col=c('blue','red'),type='l',ylab="Maturity",xlab="Age",main="Previous maturity, OMI_6")
legend('bottomright',legend=c("East","West"),text.col=c('blue','red'),bty='n')
```

In this example we're going to copy the *OMI\_6* object, and change maturity to a logistic model of selectivity with an age at 50% maturity (inflection point) of 7 in the East and 10 in the West, and slope of 1.5 for both stocks:


```{r Step_3_OMI_6_new_mat,fig.width=4, fig.height=4}
OMI<-OMI_6 # copy the object
OMI@Name<-"Logistic maturity 7/10"
OMI@mat[1,,]= 1/(1+exp(7-(1:OMI@na))/1.5)
OMI@mat[2,,]= 1/(1+exp(10-(1:OMI@na))/1.5)
matplot(t(OMI@mat[,,55]),col=c('blue','red'),type='l',ylab="Maturity",xlab="Age",main="New maturity, OMI")
legend('bottomright',legend=c("East","West"),text.col=c('blue','red'),bty='n')
```
...
```{r Step_3_write_M3}
M3dir<-"C:/ABT-MSE/M3"   # Or whereever you put the /ABT-MSE/ directory on your computer
M3write(OMI,M3dir)
```
...
If you are planning on running this model again it may be beneficial to store the final parameter estimates as initial values so that the optimization starts reasonably close to the solution an therefore is much faster to run. To do this use the pin\_from\_par() function which creates a .pin file in your M3dir directory:
...
*Note: this can be done manually by navigating to the folder and running the executable which while clunky, means you don't have to wait for the runM3() function to stop and you can keep working in R*
...
```{r Step_3_run_M3_parallel,eval=F}
 M3dir<-paste0("C:/myOMfolders/",1:8)  # An example, these folders don't really exist
 M3dir
 sfInit(parallel=TRUE, cpus=detectCores()) 
 #sapply(1:length(M3dir),runM3p,OMdir=M3dir)                      # Lets not actually do this!
 #make_fit_reports(M3dir)           # Or this!
...
```{r Step_3_make_OM_custom_proyears_etc,eval=F}
 OM_example<-new('OM',M3dir,nsim=200,proyears=30,Snames=c("Med","GOM"))
...
The first list item is called *$proyears*. This defines when recruitment changes in the future. *proyears* is an array with n time period rows (in this case 2, projection years 1:11 and 12 onwards) and 2 columns showing when these projection time periods start and end. 

In this case we can see time period 1 running from projection year 1 to projection year 11 and time period 2 (row 2) running from projection year 12 to infinity. 
...
If *Recruitment* is not specified, the operating model fits Beverton-Holt stock recruitment relationships to all historical years (estimating both steepness and R0) and assumes the same recruitment dynamics in the future (just one time period).
...
All MPs however have the same basic structure: they operating on a simulation number *x* of a simulated dataset object *dset* and return a single TAC recommendation. 

Lets look at a relative simple MP, Islope1:
...
Many MPs use numerical optimization to fit models to data. A good example are simple stock assessments such as surplus production models. While very simple, surplus production assessments are very simple and are still used to provide management advice for Atlantic tunas and billfish, for example BSP (McAllister et al. ) and ASPIC. 
...
Using the *Y30* PM we can compare these runs for the MPs tested:

```{r Allocation_yield_comp}
apply(Y30(MSE_default,pp=1),1,mean)
apply(Y30(MSE_alt_MPareas,pp=1),1,mean)
apply(Y30(MSE_alt_Allocation,pp=1),1,mean)
...
MSE_example@MPs
plot(MSE_example@Cb,Y30(MSE_example,pp=1)[3,],xlab="Catch bias",ylab="Long term yield",main="Delay difference assessment performance",col='blue')
...
plot(MSE_example@SSB0[,1],Y30(MSE_example,pp=1)[3,],xlab="Unfished spawning biomass",ylab="Long term yield",main="Delay difference assessment performance",col='blue')
...
out<-M3read("C:/ABT-MSE/M3") # get the raw M3 model fit 
load("C:/ABT-MSE/M3/OMI")    # load the corresponding OMI object in the same folder
...
A summary can be found [here](https://docs.google.com/spreadsheets/d/ 13pFaM3BTnzQ1BNQGoYn4O2n1IeD18V3VTbN9Hv7139U/edit).
